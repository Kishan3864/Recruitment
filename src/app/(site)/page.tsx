...
import { getServices, getTestimonials } from "@/lib/content/collections";
import { getFeaturedJobs } from "@/lib/content/jobs";
import { getBlogPage, getHomePage, getJobsPage, getServicesPage } from "@/lib/content/pages";
import { getClientLogos } from "@/lib/content/site";

/** Home — statically rendered composition of content-driven sections. */
export default async function HomePage() {
  const [home, services, logos, featuredJobs, testimonials, posts] = await Promise.all([
    getHomePage(),
    getServices(),
    getClientLogos(),
    getFeaturedJobs(3),
  ...
  const [servicesPage, jobsPage, blogPage] = await Promise.all([
    getServicesPage(),
    getJobsPage(),
    getBlogPage(),
  ]);
  ...
  return (
    <>
      ...
      <FeatureGrid intro={home.whyUs} features={home.whyUs.features} sunken />
      <JobsPreview
